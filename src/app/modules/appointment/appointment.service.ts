...
const cancelUnpaidAppointments = async () => {
    ...
    const appointmentIdsToCancel = unPaidAppointments.map((appointment: { id: string }) => appointment.id);

    interface IUnPaidAppointment {
        id: string;
        doctorId: string;
        scheduleId: string;
    }

    await prisma.$transaction(async (tnx: Prisma.TransactionClient) => {
        ...
        for (const unPaidAppointment of unPaidAppointments as IUnPaidAppointment[]) {
            await tnx.doctorSchedules.update({
                where: {
                    doctorId_scheduleId: {
...
const createAppointmentWithPayLater = async (user: IAuthUser, payload: any) => {
    ...
    type IAppointmentWithRelations = Prisma.AppointmentGetPayload<{
        include: { patient: true; doctor: true; schedule: true };
    }>;

    const result: IAppointmentWithRelations = await prisma.$transaction(async (tnx: Prisma.TransactionClient) => {
        const appointmentData: IAppointmentWithRelations = await tnx.appointment.create({
            data: {
                patientId: patientData.id,
                doctorId: doctorData.id,
                scheduleId: payload.scheduleId,
...
const initiatePaymentForAppointment = async (appointmentId: string, user: IAuthUser) => {
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        success_url: `${process.env.FRONTEND_URL || 'http://localhost:3000'}/payment/success`,
        cancel_url: `${process.env.FRONTEND_URL || 'http://localhost:3000'}/dashboard/my-appointment`,
    });
